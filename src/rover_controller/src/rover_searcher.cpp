...
private:
    ...
    void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
    {
        ...
        if (phase_ == 0) // Phase 0 - Initialize position
        {
            ...
            next_angle = initial_yaw_;
            phase_ = 1;
            update_to_next_vertex();
            return;
        }
        if(phase_ == 2)
        {
    ...
    bool reached_target_vertex(double current_x, double current_y)
    {
        return get_euclidean_distance(current_x, current_y, goal_x_, goal_y_) < 0.1;
    }

    void update_to_next_vertex()
    {     
        goal_x_ = current_x_ + side_length_ * std::cos(next_angle);
        goal_y_ = current_y_ + side_length_ * std::sin(next_angle);
        RCLCPP_INFO(this->get_logger(), "next vertex");
    ...
    void control_loop()
    {
        ...
        {
            if (!reached_target_vertex(current_x_, current_y_))
            {
                RCLCPP_INFO(this->get_logger(), "Phase 1");
                desired_yaw = std::atan2(goal_y_ - current_y_, goal_x_ - current_x_);
                double yaw_diff = wrap_to_pi(desired_yaw - current_yaw_);

                if (std::abs(yaw_diff) > 0.1)
                {
                    message.linear.x = 0.0;
                    message.angular.z = (yaw_diff > 0) ? turning_speed : -turning_speed;
                }
                else
                {
                    message.linear.x = forw_speed;
        ...
        {
            RCLCPP_INFO(this->get_logger(), "Phase 2");
            message.linear.x = 0.0;
    ...
    rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr aruco_gateway_subscription_;
